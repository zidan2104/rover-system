...
class VideoStreamingWidget(QWidget):
    ...
    # Sinyal toggle fullscreen (di-emit dari thread mpv, dijalankan di main thread)
    ...
    def __init__(self, parent=None):
        ...
        # State fullscreen (double-click video → fullscreen → double-click → normal)
        self._fullscreen   = False
        self._host_layout  = None     # diisi oleh create_video_panel
        self._host_index   = 0
        self.fullscreen_toggle.connect(self.toggle_fullscreen)
    ...
    def _ensure_mpv(self):
        ...
        try:
            ...
            # Double-click di video → toggle fullscreen. mpv yang punya window native,
            # jadi binding di mpv paling andal. Callback jalan di thread mpv → emit
            # sinyal Qt agar toggle dijalankan di MAIN thread (wajib untuk GUI).
            try:
                # python-mpv memanggil handler dgn 5 arg (state,name,char,scale,arg)
                # → pakai *args. state diawali 'd' untuk event 'down' (klik).
                self.mpv.register_key_binding(
                    'MBTN_LEFT_DBL',
                    lambda state, *args: (
                        self.fullscreen_toggle.emit() if str(state).startswith('d') else None))
            except Exception as e:
                print("[VIDEO] dblclick binding gagal:", e)
        except Exception as e:
    ...
    def _teardown_mpv(self):
        """Matikan instance mpv (lepas native window) — WAJIB sebelum reparent
        supaya tidak deadlock/freeze & tidak black (winId X11 berubah saat reparent)."""
    ...
    def toggle_fullscreen(self):
        """Double-click video: panel ↔ fullscreen 1 monitor.

        Reparent widget di X11 MENGHANCURKAN winId video_frame, jadi mpv harus
        dibongkar dulu (cegah freeze/black) lalu dibuat ULANG ke winId baru +
        replay stream. Ada blip singkat saat toggle — itu wajar & aman."""
        # Debounce: mpv-binding & Qt-event bisa sama-sama memicu → cukup 1x.
        now = time.time()
        if now - getattr(self, "_last_toggle", 0.0) < 0.4:
            return
        self._last_toggle = now

        self._was_streaming = self.is_streaming
        self._teardown_mpv()            # 1) lepas mpv dulu (anti freeze/black)

        if not self._fullscreen:        # 2) panel → fullscreen
            if self._host_layout is not None:
                self._host_index = self._host_layout.indexOf(self)
                self._host_layout.removeWidget(self)
            self.setParent(None)        # jadi window top-level
            self.showFullScreen()
            self._fullscreen = True
        else:                           # 2) fullscreen → balik ke panel
            if self._host_layout is not None:
                self._host_layout.insertWidget(self._host_index, self)
            self.showNormal()
            self.show()
            self._fullscreen = False

        self.setFocus()
        # 3) Buat ulang mpv SETELAH event loop me-realize & memetakan winId baru
        #    (kalau langsung, video_frame belum termap → mpv render ke window
        #    kosong → black saat balik ke panel). 150 ms cukup utk layout settle.
        QTimer.singleShot(150, self._rebuild_video)

    def _rebuild_video(self):
        """Pasang ulang mpv ke video_frame yang sudah termap (dipanggil via timer)."""
        self.video_frame.setGeometry(0, 0, self.width(), self.height())
        self.video_frame.show()
        if not getattr(self, "_was_streaming", False):
            return
        self._ensure_mpv()              # baca winId BARU yang sudah valid
        if self.mpv is not None:
            try:
                self.mpv.play(self.STREAM_URL)
            except Exception as e:
                print("[VIDEO] replay after fullscreen error:", e)
        self._update_osd()

    def mouseDoubleClickEvent(self, event):
        # Cadangan kalau mpv tak menangkap (klik di area non-video); debounce di toggle.
...
if GUI_AVAILABLE:
    ...
    class SpacebotGCS(QMainWindow):
        ...
        def create_video_panel(self):
            ...
            self.video_widget._host_layout = layout   # untuk restore dari fullscreen
